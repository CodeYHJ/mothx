...
try {
  ...
  await evaluate(cdp, `document.querySelector('.trajectory-segmented button:last-child')?.click()`);
  assert.equal(await evaluate(cdp, `document.querySelector('.trajectory-segmented button:last-child')?.classList.contains('active')`), true);

...
2. **项目允许规则**：命令匹配 `.mothx/allow.json` 中的 `bashCommands`（精确）或 `bashPrefixes`（前缀）→ 自动批准。
...
- **审批一次** — 仅此次执行该命令。
- **拒绝** — 拒绝该命令。
- **始终允许此命令** — 将精确命令持久化到 `.mothx/allow.json` 并批准。
- **始终允许命令前缀** — 将命令前缀（如 `go test `）持久化到 `.mothx/allow.json` 并批准。

导航方式：↑/↓ 移动、Enter 选择、y 批准、n 拒绝、Esc 中止。

### 项目级允许规则（`allow.json`）

项目允许规则保存在 `.mothx/allow.json` 中，支持：
...
3. **使用项目允许规则**: 优先使用 `.mothx/allow.json` 而非全局白名单管理项目特定命令
